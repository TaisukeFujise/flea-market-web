...
| left | file | ✓ | 左側面画像 |
| top | file | ✓ | 上面画像 |
| category_id | string (UUID) | ✓ | カテゴリID。同カテゴリの過去傷報告を pgvector 検索して傷検出プロンプトの参照例に追加する |
...
| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| image_id | string (UUID) | ✓ | 商品の出品画像（product_images）のID |
| damage_type | string | ✓ | `scratch` / `dirt` / `wear` のいずれか |
| bbox_x1 | integer | ✓ | 傷箇所の左上 X 座標（0-1000 正規化座標） |
| bbox_y1 | integer | ✓ | 傷箇所の左上 Y 座標（0-1000 正規化座標） |
| bbox_x2 | integer | ✓ | 傷箇所の右下 X 座標（bbox_x1 より大きい値） |
| bbox_y2 | integer | ✓ | 傷箇所の右下 Y 座標（bbox_y1 より大きい値） |
| description | string | — | 傷の説明（任意） |
