...
type PaginationAction =
  | { type: 'fetch_start' }
  | { type: 'fetch_done'; items: LikeItem[]; total: number }
  | { type: 'fetch_error' }
  | { type: 'retry' }
...
function initPagination(loaderData: LikesLoaderData): PaginationState {
  return {
...
export default function LikesPage() {
  ...
  const fetchMore = useCallback(async () => {
    ...
    try {
      const data = await apiFetch<Paginated<LikeItem>>(`/api/me/likes?limit=20&offset=${offsetRef.current}`)
